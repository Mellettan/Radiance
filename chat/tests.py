...
class ChatViewTests(TestCase):
    ...
    def setUp(self):
        ...
        """
        self.user1 = CustomUser.objects.create_user(email='user1@example.com', username='user1', password='password1')
        self.user2 = CustomUser.objects.create_user(email='user2@example.com', username='user2', password='password2')
        self.user3 = CustomUser.objects.create_user(email='user3@example.com', username='user3', password='password3')

        Message.objects.create(sender=self.user1, recipient=self.user2, content="Hello user2 from user1")
        Message.objects.create(sender=self.user2, recipient=self.user1, content="Hello user1 from user2")
    ...
    def test_chat_view_with_messages(self):
        ...
        self.assertQuerySetEqual(
            get_chat_users(self.user1),
            response.context['chat_users']
        )
    ...
    def test_chat_view_with_no_messages(self):
        ...
        self.assertQuerySetEqual(CustomUser.objects.none(), response.context['message_list'])
        self.assertQuerySetEqual(
            get_chat_users(self.user1),
            response.context['chat_users']
        )
