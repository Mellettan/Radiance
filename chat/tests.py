...
class ChatViewTests(TestCase):
    ...
    def test_login_required(self):
        ...
        Выполняет GET-запрос к странице чата для другого пользователя без
        аутентификации и проверяет:
          - Переадресацию на страницу входа с параметром next, указывающим
            на страницу чата.
        """
    ...
    def test_chat_view_with_messages(self):
        ...
        Выполняет GET-запрос к странице чата для пользователя `user1`,
        предварительно войдя в систему, и проверяет:
          ...
          - message_list: Список сообщений между пользователями, отсортированный по
            времени отправки.
    ...
    def test_chat_view_with_no_messages(self):
        ...
        Выполняет GET-запрос к странице чата для пользователя 1 с другим пользователем,
        с которым нет сообщений, и проверяет:
          - Код статуса ответа (должен быть 200).
          - Используемый шаблон (должен быть 'chat/chat.html').
          - Корректность данных в контексте ответа:
          - other_user: Пользователь, с которым ведется чат.
