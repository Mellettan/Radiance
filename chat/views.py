...
def get_chat_users(user: CustomUser) -> QuerySet:
    """
    Получает отдельный список пользователей, с которыми данный пользователь
    ...
    Returns:
        QuerySet: Отдельный список экземпляров `CustomUser`,
            упорядоченный по имени пользователя.
    """
...
class ChatView(LoginRequiredMixin, View):
    """
    Представление для отображения интерфейса чата между вошедшим в систему пользователем
    и другим пользователем.

    Это представление извлекает и отображает сообщения чата, которыми обмениваются
    текущий пользователь и еще один пользователь, идентифицированный `user_id`.
    Он также предоставляет чаты текущего пользователя.
    """

    def get(self, request: HttpRequest, user_id: int) -> HttpRequest:
        """
        Обрабатывает запросы GET для отображения интерфейса чата с указанным
        пользователем.

        Args:
            request (HttpRequest): Объект запроса, используемый для генерации этого
                ответа.
