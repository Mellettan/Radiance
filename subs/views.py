...
class SubsView(View):
    ...
    Это представление обрабатывает запросы GET для отображения списка подписок пользователя и поиска
    пользователей. Оно также поддерживает пагинацию для списка пользователей.
    """
    ...
    def get(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        Проверяет, авторизован ли пользователь. Если да, то извлекает и отображает его подписки. Выполняет поиск
        пользователей по запросу, если таковой имеется. Реализует пагинацию для списка пользователей.
