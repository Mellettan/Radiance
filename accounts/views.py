...
class LoginView(FormView):
    """
    Представление, которое обрабатывает вход пользователя с помощью настраиваемой формы входа.
...
class ChangeEmailView(FormView):
    """
    Представление, которое обрабатывает процесс изменения адреса электронной почты пользователя.

    Attributes:
        template_name (str): Путь к шаблону, который отображает форму изменения адреса электронной почты.
    ...
    def form_valid(self, form: ChangeEmailForm) -> HttpResponseRedirect:
        ...
        Этот метод выполняет следующие действия:
            1. Генерирует токен для подтверждения по электронной почте, используя `default_token_generator`.
            2. Кодирует первичный ключ пользователя («uid») для безопасного использования в URL-адресах.
            3. Создает ссылку подтверждения с токеном и `uid`.
            4. Отправляет электронное письмо с подтверждением и сгенерированной ссылкой на новый адрес электронной почты.
            5. Сохраняет новый адрес электронной почты в поле temp_email объекта пользователя.
            6. Отображает информационное сообщение о том, что ссылка для подтверждения отправлена.
        """
...
class ConfirmEmailView(View):
    ...
    def get(
        ...
        Returns:
            HttpResponseRedirect: Перенаправляет пользователя на определенный URL-адрес после обработки подтверждения.

        Этот метод выполняет следующие действия:
            1. Декодирует идентификатор пользователя (uidb64) для извлечения соответствующего пользователя из базы данных.
            2. Проверяет, существует ли пользователь и действителен ли токен.
            3. Если токен действителен и у пользователя есть временный адрес электронной почты, адрес электронной почты подтверждается:
               - Для параметра `email_verified` устанавливается значение True.
               — Временный адрес электронной почты («temp_email») перемещен в основное поле «email».
               — Поле temp_email очищается.
            4. Если подтверждение прошло успешно, отобразится сообщение об успехе. Если нет, отображается сообщение об ошибке.
