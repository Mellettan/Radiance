...
class CustomUserManager(BaseUserManager):
    ...
    def create_user(
        ...
        """
        Создаёт и сохраняет пользователя с указанным адресом электронной
        почты и паролем.
...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    Attributes:
        ...
        avatar (ImageField): Изображение аватара пользователя с ограничениями
            на типы файлов.
        email_verified (BooleanField): Указывает, подтвержден ли адрес электронной
            почты пользователя.
        temp_email (EmailField): Временный адрес электронной почты для проверки
            электронной почты.
        date_joined (DateTimeField): Дата и время создания учетной записи пользователя.
        USERNAME_FIELD (str): Поле, которое будет использоваться в качестве уникального
            идентификатора для аутентификации (электронная почта).
        REQUIRED_FIELDS (list): Поля, которые будут запрашиваться
            (помимо USERNAME_FIELD) при создании пользователя через командную строку.
        objects (CustomUserManager): Менеджер, который занимается созданием
            экземпляров пользователей и управлением ими.
    """
...
class Subscription(models.Model):
    """
    Модель подписки. Представляет отношения подписки между двумя пользователями в
    системе, где один пользователь подписан на другого.
