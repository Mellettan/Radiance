...
def send_confirmation_email(
    ...
    Эта функция использует Dramatiq для асинхронной обработки процесса отправки
    электронной почты.
    Это означает, что электронное письмо будет отправлено в фоновом режиме, не блокируя
    ...
    Note:
        - Аргумент `fail_silly=False` гарантирует, что любые исключения во время
          процесса отправки электронной почты будут вызваны, что облегчит
          отладку проблем.
        - `settings.DEFAULT_FROM_EMAIL` используется в качестве адреса
