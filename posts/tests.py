...
class HomeViewTest(TestCase):
    """
    Тесты для представления домашней страницы пользователя,
    включая создание постов, комментариев, лайков и подписок.

    Этот класс содержит тесты для проверки функциональности домашней страницы,
    такие как создание постов, комментариев, лайков и подписок.
    Он также проверяет, что при выполнении различных действий возвращаются
    ...
    def setUp(self):
        ...
        Создает двух пользователей, пост и подписку между пользователями.
        Эти данные используются в тестах для проверки функциональности
        домашней страницы.
        """
    ...
    def test_get_home_page(self):
        ...
        Этот тест проверяет, что домашняя страница пользователя загружается успешно,
        используется правильный шаблон и передается необходимый контекст.
        """
    ...
    def test_create_post(self):
        ...
        Этот тест проверяет, что после отправки формы для создания нового поста,
        пост добавляется в базу данных и отображается на странице.
        """
    ...
    def test_create_comment(self):
        ...
        Этот тест проверяет, что после отправки формы для создания комментария,
        комментарий добавляется к посту и отображается на странице.
        """
    ...
    def test_like_unlike_post(self):
        ...
        Этот тест проверяет, что пользователь может поставить лайк и снять лайк с поста.
        Проверяется, что состояние лайка меняется при повторной отправке запроса.
        """
        self.client.force_login(self.user2)
        url = reverse("posts:home", kwargs={"pk": self.user1.pk})
    ...
    def test_subscribe_unsubscribe_user(self):
        ...
        Этот тест проверяет, что пользователь может подписаться на другого пользователя
...
class ScheduleBotPostsCommandTest(TestCase):
    ...
    Этот класс содержит тесты для команды `schedule_daily_posts`, которая планирует
    создание постов для ботов в случайное время в течение дня.
    Тесты проверяют, что посты планируются правильно и что только боты получают
    задачи на создание постов.
    """
    ...
    def setUp(self):
        ...
        Создаются два бота и один обычный пользователь.
        Эти данные используются для проверки функциональности команды.
        """
    ...
    def test_schedules_posts_for_bots(
        ...
        Этот тест проверяет, что для каждого бота создаются три задачи на создание
        постов в случайные моменты времени утром, днём и вечером.
        Используются фиктивные значения для проверки правильности планирования задач.

        Args:
            mock_randint (MagicMock): Мок для функции random.randint,
                возвращает фиксированные значения.
            mock_send_with_options (MagicMock): Мок для метода
        ...
        # Проверяем, что create_bot_post.send_with_options был вызван 6 раз
    ...
    def test_no_tasks_for_non_bots(
        ...
        """
        Проверяет, что команда не планирует задачи на создание постов для
        обычных пользователей.

        Этот тест проверяет, что команда `schedule_daily_posts`
        не планирует задачи для обычных пользователей.
