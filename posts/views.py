...
class HomeView(View):
    ...
    def get(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        subscriptions = Subscription.objects.filter(follower=user_of_posts).select_related('following')
        posts = Post.objects.filter(user=user_of_posts).select_related('user').prefetch_related(
            'comments').prefetch_related('comments__user').prefetch_related('liked_by').order_by('-created_at')
