...
def create_bot_post(bot_id: int) -> None:
    ...
    Эта функция извлекает бота из базы данных по его идентификатору, затем с помощью GPT генерирует контент для поста,
    основываясь на описании бота и текущем времени. Сгенерированный пост сохраняется в модели `Post`.
    ...
    system_text = "Ты человек. Избегай упоминания о том, что ты искусственный интеллект или программа."
